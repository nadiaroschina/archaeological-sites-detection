...
def train_wgan_spectral(results_dir, pbar, opt, generator, discriminator, dataloader, device, lambda_gp=10):
    ...
    for epoch in range(opt['n_epochs']):
        ...
        for i, (imgs, indices, coords, labels) in enumerate(dataloader):
            ...
            fake_losses.append(torch.mean(fake_validity).item())
            real_losses.append(torch.mean(real_validity).item())
            gp_losses.append((lambda_gp * gradient_penalty).item())
            ...
            if i % opt['n_critic'] == 0:
                ...
                if batches_done % opt['sample_interval'] == 0:
                    ...
                    axes[0].plot(discriminator_losses, label='discriminator loss')
                    axes[0].plot(fake_losses, label='fake loss')
                    axes[0].plot(real_losses, label='real loss')
                    axes[0].plot(gp_losses, label='gp loss')
                    axes[1].plot(generator_losses, label='generator loss')
                    
                    y_min = max([max(discriminator_losses[-1000:]), max(fake_losses[-1000:]), max(real_losses[-1000:]), max(gp_losses[-1000:])])
                    y_max = min([min(discriminator_losses[-1000:]), min(fake_losses[-1000:]), min(real_losses[-1000:]), min(gp_losses[-1000:])])
                    dy = y_max - y_min
                    axes[0].set_ylim(y_min - 0.5 * dy, y_max + 0.5 * dy)
